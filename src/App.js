...
function TradingJournalApp() {
  ...
  useEffect(() => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ entries, customChecks })
    );
  ...
  const allWeeks = useMemo(() => {
    const rows = Object.keys(entries).map((k) => {
      ...
      const y =
        Object.values(item.answers || {}).filter(Boolean).length +
        Object.values(item.customAnswers || {}).filter(Boolean).length;
      const t =
        Object.keys(item.answers || {}).length + (customChecks?.length || 0);
    ...
    return rows.filter(
      (r) => !filter || r.start.includes(filter) || r.end.includes(filter)
    );
  ...
  const handleAddScreenshots = async (files) => {
    ...
    for (const f of files) {
      ...
      if (f.size > maxEachMB * 1024 * 1024) {
        alert(`${f.name} is larger than ${maxEachMB} MB; skipping.`);
        continue;
      }
      const dataUrl = await fileToDataUrl(f);
      newShots.push({
        id: `shot_${Date.now()}_${Math.random().toString(36).slice(2)}`,
        name: f.name,
        dataUrl,
      });
    }
  ...
  const parseMT5CSV = (text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const dt = parseMT5Date(get(timeIdx));
      if (!dt) continue;
      out.push({
        time: dt,
  ...
  const handleImportMT5 = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        alert("MT5 import failed: " + err.message);
      }
    };
      ...
      <div style={cardStyle}>
        ...
        >
          ...
          <div>
            ...
            >
              {showPrintView ? "Exit Print View" : "Print View"}
            </button>
            <button
              style={buttonStyle}
              onClick={() => {
                ...
                a.href = url;
                a.download = "trading_journal_backup_v2.json";
                a.click();
        ...
        <section style={{ display: "grid", gap: 16, marginBottom: 24 }}>
          <div style={sectionStyle}>
            <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>
              Pick any date within the week you want to journal
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              ...
              <Pill>
                <span style={{ fontWeight: 600 }}>Trades:</span>
                <span>{current.stats?.numberOfTrades ?? 0}</span>
              </Pill>
              <Pill>
                <span style={{ fontWeight: 600 }}>PnL:</span>
                <span>{(current.stats?.pnl ?? 0).toFixed(2)}</span>
              </Pill>
            </div>
          </div>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea
              placeholder="What happened this week? setups taken, market context, emotions, lessons..."
              style={{ ...inputStyle, width: "100%", height: 120 }}
          ...
          >
            ...
            <div>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                <Toggle
                  label="Did you put the stop-loss where it was supposed to be?"
                  hint="Per your plan / OB invalidation"
                  checked={current.answers.stopLossPlaced}
                  onChange={(v) =>
                    updateCurrent({
                      answers: { ...current.answers, stopLossPlaced: v },
                    })
                  }
                />
                <Toggle
                  ...
                  onChange={(v) =>
                    updateCurrent({
                      answers: { ...current.answers, revengeTradeAfterOneSL: v },
                    })
                  }
                />
                <Toggle
                  label="Did you follow the risk management?"
                  hint="Position size, 1R, max daily loss, no over-leverage."
                  checked={current.answers.followedRiskManagement}
                  onChange={(v) =>
                    updateCurrent({
                      answers: { ...current.answers, followedRiskManagement: v },
                    })
                  }
                />
              </div>
              ...
              >
                <div>
                  ...
                  <input
                    type="number"
                    min={0}
                    value={current.stats?.numberOfTrades ?? 0}
                    onChange={(e) =>
                      updateCurrent({
                        stats: {
                          ...current.stats,
                          numberOfTrades: Math.max(
                            0,
                ...
                <div>
                  ...
                  <input
                    type="number"
                    step="0.01"
                    value={current.stats?.pnl ?? 0}
                    onChange={(e) =>
                      updateCurrent({
                        stats: { ...current.stats, pnl: Number(e.target.value || 0) },
                      })
                    }
                    style={{ ...inputStyle, width: "100%" }}
                  />
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
              <input
                type="text"
                placeholder="Type a tag and press Enter (e.g., XAUUSD, London, OB)"
                ...
                {Object.keys(entries).length === 0 && (
                  <tr>
                    <td colSpan={6} style={{ padding: 18, textAlign: "center", color: "#6b7280" }}>
                      No entries yet. Start with “Use This Week” above.
                    </td>
                  </tr>
                )}
                {allWeeks.map((w) => (
        ...
        <footer style={{ textAlign: "center", fontSize: 12, color: "#6b7280", marginTop: 16 }}>
          Built for your trading routine • ISO week (Mon–Sun) • Data stays in your browser (localStorage)
        </footer>
      </div>
